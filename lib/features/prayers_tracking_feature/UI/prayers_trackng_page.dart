import 'package:app/ads/after_prayer_ad.dart';
...
class PrayersTrackngPage extends StatefulWidget {
  const PrayersTrackngPage({super.key, required this.prayers});
  final List<Map<String, dynamic>> prayers;

  @override
  State<PrayersTrackngPage> createState() => _PrayersTrackngPageState();
}

class _PrayersTrackngPageState extends State<PrayersTrackngPage> {
  @override
  void initState() {
    super.initState();
    AfterPrayerAd.loadInterstitial();
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) {
        if (didPop) return;
        AfterPrayerAd.showInterstitial(() {
          Navigator.of(context).pop();
        });
      },
      child: MultiBlocProvider(
        providers: [
          BlocProvider(create: (context) => PrayerTimeCubit(widget.prayers)),
          BlocProvider(
            create: (context) =>
                PrayersTrackingCubit()..loadPrayers(widget.prayers),
          ),
        ],
        child: Scaffold(
          body: Column(
